...
class UserAppointments(View):
    ...
    def get(self,request,user_id,*args,**kwargs):
        ...
        if search:
            appointments = user.user_appointments.filter(Q(doctor__name__icontains=search) | Q(doctor__family__icontains=search) | Q(doctor__phone_number__icontains=search))
        else:
            appointments = user.user_appointments.all()
